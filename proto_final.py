"""Prototype final: layer-transparent recursion + dynamic-route-param guard
+ TRUE owned_max (residual de-owned to role=shared, not counted)."""
...
_DEPTH_CAP = 6
_DYNAMIC_RE = re.compile(r"^\[.*\]$|^_")  # [x] [[x]] [...x] and _private
...
def plan_split(owned, floor):
    start = _common_segs(owned)
    raw = defaultdict(list)
    residual = []
    for p in owned:
        k = domain_of(p, start)
        (residual.append(p) if k is None else raw[k].append(p))
...
for slug in ["caddy", "inbox-zero", "formbricks"]:
    ...
    for f in feats:
        ...
        if not domains:
            if o:
                new_owned.append((set(o), f.get("name")))
            continue
        # source keeps NOTHING owned from the split set (residual -> shared).
        # (real impl: residual stays in member_files as role=shared, primary=False)
        n_subs_total += len(domains)
        split_report.append((f.get("name"), len(o), len(domains), len(residual),
                             sorted({leaf(k) for k in domains})))
        for k, files in domains.items():
            new_owned.append((set(files), leaf(k)))
    ...
    print(f"  AFTER  owned_max={after_max:.3f} ({after_big})   subs_created={n_subs_total}")
    for nm, no, nd, nr, names in split_report:
        print(f"    [{nm} owned={no}] -> {nd} subs, residual(->shared)={nr}")
        print(f"       {names}")
