...
_MANIFEST_SKIP_DIRS = frozenset({
    ...
    # Sibling-app / docs / examples dirs whose own manifests pull in
    # deps unrelated to the primary library/product. Walking them
    # poisons the anchor signal (e.g. better-auth/docs/package.json
    # depends on @vercel/analytics → critique invents an Analytics
    # feature for the auth library). These are universal naming
    # conventions across thousands of repos — not repo-specific.
    "docs", "documentation", "doc",
    "examples", "example", "samples", "sample",
    "demo", "demos", "playground",
    "website", "site", "www", "landing", "marketing",
    "sandbox", "tutorial", "tutorials",
    "docs_src", "cookbook", "snippets",
})
...
def find_manifests(repo_root: Path) -> list[Path]:
    ...
    Skips vendored/build dirs and sibling-app/docs/example dirs whose
    manifests would otherwise leak unrelated deps into the anchor map.
    """
    out: list[Path] = []
    for filename in _MANIFEST_PARSERS:
        for path in repo_root.rglob(filename):
            rel_parts = path.relative_to(repo_root).parts
            if any(p in _MANIFEST_SKIP_DIRS for p in rel_parts):
                continue
            out.append(path)
    return out
