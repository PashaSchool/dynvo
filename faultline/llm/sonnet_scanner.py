...
- Very large feature (300+ files): 10-15 flows — use the subdirectory
  breakdown to identify flows

A flow is a USER JOURNEY (multi-step task) OR a distinct API endpoint
operation. CRUD-shaped features deserve all five verbs as separate flows
(create / list / get / update / delete) when each verb has dedicated
code. Engineering audiences want this granularity — they map flows to
PRs and tests.

A downstream consolidator merges verb-synonym duplicates
(configure/setup/manage → one) and caps at 12 flows per feature, so
do NOT under-emit defensively — emit every distinct journey you see
and let the consolidator clean up.
...
_STACK_GUIDANCE: dict[str, str] = {
    ...
    "rails-app": (
        ...
        "feature named after the resource. Two controllers in the "
        "SAME parent directory and clearly serving the same domain "
        "may merge into one feature; controllers in DISTINCT "
        "subdirectories under ``app/controllers/`` are SEPARATE "
        "features (different namespaces typically reflect different "
        "product surfaces such as web vs API vs admin).\n"
        "2. Matching ``app/models/<resource>.rb`` and "
        "``app/views/<resource>/`` belong to the same feature as the "
        "controller.\n"
        "3. Generic base classes (``ApplicationController``, "
        "``ApplicationRecord``, ``ApiController``, anything ending in "
        "``BaseController``) and files under ``app/controllers/concerns/`` "
        "→ ``shared-infra``.\n"
        "4. ``app/jobs/``, ``app/mailers/`` group by what they serve — "
        "a mailer named after a resource joins that resource's feature.\n"
        "5. Engines mounted under ``app/controllers/<engine>/`` (e.g. "
        "``app/controllers/doorkeeper/``, ``app/controllers/api/v1/``) "
        "form their own feature OR a small set of features grouped by "
        "the engine name — do not fold them into a sibling feature.\n\n"
