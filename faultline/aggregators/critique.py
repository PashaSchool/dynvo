...
def _category_from_signal(
    ...
    if sig.kind == "python-framework-module":
        # Top-level .py module inside a self-shipped Python web
        # framework (fastapi/routing.py, flask/blueprints.py).
        # Each meaningful module is a distinct capability of the
        # framework — Path Operations, Blueprints, Background
        # Tasks, etc. Surfaces what the bucketizer's structural
        # name filter would otherwise swallow under the framework
        # name itself.
        module = p.get("module")
        ...
        f = p.get("file", "")
        loc = p.get("loc", 0)
        ev = f"python-framework-module:{f} ({loc} loc)"
        return module, ev, "should"
