...
# ── Acceptance #4: golden cache-key stability ────────────────────────────


def test_golden_cluster_cache_key_unchanged() -> None:
    """``_cluster_cache_key`` must hash identically to the pre-refactor
    implementation — dev fs caches stay valid."""
    from faultline.llm.detector import _cluster_cache_key

    mapping = {
        "c1": ["src/auth/login.ts", "src/auth/logout.ts"],
        "c2": ["src/billing/stripe.ts"],
    }
    model = "claude-haiku-4-5-20251001"
    # Reproduce the exact legacy computation.
    import hashlib

    all_files = sorted(f for files in mapping.values() for f in files)
    expected = hashlib.sha256(
        (json.dumps(all_files) + model).encode()
    ).hexdigest()[:24]
    assert _cluster_cache_key(mapping, model) == expected


def test_golden_flow_hashes_unchanged() -> None:
    """flow_judge feature/flow-set hashes must be stable."""
    import hashlib
    ...
    features = {"auth": "d", "billing": "d"}
    expected_feat = hashlib.sha256(
        json.dumps(sorted(features.keys()), ensure_ascii=False).encode()
    ).hexdigest()[:16]
    assert _feature_set_hash(features) == expected_feat

    flows = [
        FlowEntry(name="login", current_owner="auth"),
        FlowEntry(name="checkout", current_owner="billing"),
    ]
    expected_flow = hashlib.sha256(
        json.dumps(
            sorted([(f.current_owner, f.name) for f in flows]),
            ensure_ascii=False,
        ).encode()
    ).hexdigest()[:16]
    assert _flow_set_hash(flows) == expected_flow
